""" Swin Transformer
This version rectified the asynchronous problem in the scalar + exp(theta * (xavgi - xavgavg)) problem
That is, exp function causes different updating speed between the "scalar" and "exp(theta)", this may lead un-alignment problem
"""
# --------------------------------------------------------
# Swin Transformer
...
# version: csan v1 (on swin window attention)
...
# --------------------------------------------------------
# CSAN attention adapted for CoFormer (sequence-first)
# --------------------------------------------------------
...
class CSANMultiHeadSelfAttention(nn.Module):
    ...
    Inputs/outputs match `MultiHeadSelfAttention` in `models/components/attention.py`:
      - x: [B, L, C]
    ...
    def __init__(
        ...
        if embed_dim % 4 != 0:
            raise ValueError(f"embed_dim ({embed_dim}) must be divisible by 4 for CSAN gating.")
        ...
        if self.head_dim * self.num_heads != embed_dim:
            raise ValueError(
                f"head_dim ({self.head_dim}) * num_heads ({self.num_heads}) must equal embed_dim ({embed_dim})."
            )
        ...
        # CSAN gating networks (adapted from swin_csan's window attention, sequence-wide)
        ratio = 8
        threshold = 24
    ...
    @staticmethod
    def quasi_linear(x: torch.Tensor, eta: float) -> torch.Tensor:
    ...
    def forward(
        ...
        if c_in != self.embed_dim:
            raise ValueError(f"Unexpected x last dim {c_in}, expected {self.embed_dim}.")
        if struct_embed.shape[:3] != (bs, seqlen, seqlen):
            raise ValueError(
                f"Unexpected struct_embed shape {struct_embed.shape}, expected (B, L, L, pair_dim)."
            )
        ...
        # Masking: True means ignore (consistent with CoFormer)
        if attn_mask is not None:
            ...
            if attn_mask.dtype == torch.bool:
                ...
                # If it's an additive mask already, just add it.
                attn_logits = attn_logits + attn_mask

        if key_pad_mask is not None:
            # key_pad_mask True means ignore keys
        ...
        ind = qscore * u.sigmoid()  # [B, H, L, Dh/4]
        ind = ind.transpose(1, 2).reshape(bs, seqlen, -1)  # [B, L, C/4]

        indm = self.reduce(ind)  # [B, L, hidden_dim]
        indm = self.act(indm)

        ka = tanh(self.ka(indm)).view(bs, seqlen, self.num_heads, self.head_dim).transpose(1, 2) + 1.0
        kv = tanh(self.kv(indm)).view(bs, seqlen, self.num_heads, self.head_dim).transpose(1, 2)

        temp = tanh(self.temp(indm)).transpose(1, 2).unsqueeze(-1) + 1.0  # [B, H, L, 1]
        magk = tanh(self.magk(indm)).transpose(1, 2).unsqueeze(-2) + 1.0  # [B, H, 1, L]
        ...
        struct_output = struct_embed + self.outer_linear(
            outer_product.reshape(list(outer_product.shape[:-2]) + [-1])
        )
